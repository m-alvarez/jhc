...
generateC fs ss = ans where
    G ga = do
        ...
        -- let shead = vcat [ text "struct" <+> tshow (structureName s) <> (if structureAligned s then "A_ALIGNED" else empty) <> text ";" | s <- ss ]
        -- CHANGE: removed A_ALIGNED directive (it is not recognized by CBMC)
        let shead = vcat [ text "struct" <+> tshow (structureName s) <> text ";" | s <- ss ]
