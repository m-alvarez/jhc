...
compileGrinToC grin = do
    ...
    forM_ [("HsFFI.h",hsffi_h),
           ...
           ("rts/gc.h",gc_h),
           -- CHANGE: support for jgc stub
           ("rts/gc_jgc_stub.c", gc_jgc_stub_c),
           ("rts/gc_jgc_stub.h", gc_jgc_stub_h),
        ...
        --   ("rts/slub.c",slub_c),
           ...
           ("sys/bitarray.h",bitarray_h),
           ("sys/queue.h",queue_h),
           ("sys/wsize.h",wsize_h)] $ \ (fn,bs) -> do
        fileInTempDir fn $ flip BS.writeFile bs
    
    let cFiles = if fopts FO.JgcStub 
                 then ["rts/profile.c", "rts/rts_support.c","rts/jhc_rts.c", "lib/lib_cbits.c", "rts/gc_jgc_stub.c"]
                 else ["rts/profile.c", "rts/rts_support.c", "rts/gc_none.c","rts/jhc_rts.c", "lib/lib_cbits.c", "rts/gc_jgc.c","rts/stableptr.c"]
    tdir <- getTempDir
